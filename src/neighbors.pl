...
if($num_estimates eq '' || $num_estimates !~ /^\d+$/)
  {
    error("-e must be an unsigned integer less than or equal to ",
...
foreach my $set_num (0..$#$input_file_sets)
  {
    ...
    if(scalar(@$ary) < $num_estimates)
      {warning("There are fewer sequences in the sequence file: ",
	       "[$input_file] than the number of top sequences requested ",
	       "(-e): [$num_estimates].")}

    closeIn(*INPUT);

    printFirstNeighbors($ary,
			$lookup,
			$cnt,
			$num_estimates,
			$abundance_hash,
...
* WHAT IS THIS: This script finds all hamming distance 1 neighbors of a set of
                ...
                This script represents the second step of a 7 step process in
...
sub usage
  {
    ...
    if($no_descriptions)
      ...
      {
        print << 'end_print';
     -i|--seq-file*       REQUIRED Space-separated sequence input file(s).
     ...
     -q|--seq-id-pattern  OPTIONAL [^\s*[>\@]\s*([^;]+)] A perl regular
                                   ...
                                   return but including the '>' or '\@'
                                   character is available to match against.
     -e|--num-top-seqs    OPTIONAL [0] A number of the most abundant sequences
                                   in the input sequence file (-i) to use to
                                   limit the neighbor output.  Requires that
                                   the pattern supplied to -p matches the
                                   abundances on the deflines of the -i file.
                                   A value of 0 means find first neighbors of
                                   all sequences.  Note, in order to run
                                   nZeros.pl, this value must be 0, but you
                                   only need as many as are supplied with
                                   "errorRates.pl -e" if all you want to do is
                                   generate the error rates.  Note, the number
                                   of outputs is also affected by -a.  The
     ...
     -a|--minimum-        OPTIONAL [10] The abundance threshold at or above
        abundance                  which a sequence will be included as a
                                   ...
                                   The purpose of the exclusion is to speed up
                                   processing, since getCandidates.pl uses the
                                   same threshold.
...
sub printFirstNeighbors
  {
    ...
    my $seedsize   = $_[6] ? $_[6] : (defined($main::seedsize) ?  #INTERNAL
				    $main::seedsize : 0);
    ...
    #For each ary index except the last (or for all the top_seqs - if top_seqs
    #has a non-zero value) (or until the abundances are too low to matter for
    #the analysis - if size threshold is non-zero)
    for(my $i1 = 0;
	$i1 < ($cnt - 1) &&
	...
	$i1++)
      {
	verboseOverMe("Doing mother sequence [$oary->[$i1]->[0]].");
	...
	if($seedsize)
	  {
	    #Split the sequences into parts
	    @seeds = unpack("(A$seedsize)2",$oary->[$i1]->[1]);
	    if((scalar(@seeds) * $seedsize) > $len && scalar(@seeds) > 1)
	      {
		$seeds[-2] .= $seeds[-1];
		pop(@seeds);
	...
	print(join("\t",(scalar(@{$oary->[$i1]}) > 2 ?
			 @{$oary->[$i1]}[0,2..$#{$oary->[$i1]}] :
			 $oary->[$i1]->[0])),"\n");
      }

    print(join("\t",(scalar(@{$oary->[-1]}) > 2 ?
		     @{$oary->[-1]}[0,2..$#{$oary->[-1]}] : $oary->[-1]->[0])),
	  "\n") if(!$top_seqs || $top_seqs == $cnt);
