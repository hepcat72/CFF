...
my $GetOptHash =
  {'i|fasta-file=s'           => sub {push(@$input_files,#REQUIRED unless <>
   ...
   'n|neighbors-file=s'       =>  sub {push(@$ham_files, #REQUIRED
			             [sglob($_[1])])},
   'z|z-threshold=s'          => \$zthresh,              #REQUIRED
   't|filetype=s'             => \$filetype,             #OPTIONAL [auto]
				                         #   (fasta,fastq,auto)
   'p|abundance-pattern=s'    => \$abundance_pattern,    #OPTIINAL
                                                         #        [size=(\d+);]
   'e|num-estimates=s'        => \$num_estimates,        #OPTIONAL [10]
   'r|use-reverse-spillover!' => \$reverse_spillover,    #OPTIONAL [Off]
   'd|append-delimeter=s'     => \$append_delimiter,     #OPTIONAL [;N0=_VH_;]
...
foreach my $set_num (0..$#$input_file_sets)
  {
    ...
    #If the z score threshold has a value, use it to discard positions of
    #"real" substitutions from the later calculation of the background error
    #rate.
    if($zthresh)
      {
	...
	foreach my $mother_id ((sort {$abundance_hash->{$b} <=>
				...
				keys(%$abundance_hash))[0..$estimate_max])
	  {
		...
		if($z ne 'infinity' && $z > $zthresh)
		  {
		    verbose("Mother: $mother_id Neighbor: $nid Z-Score: ",
			    sigdig($z,$sigdig),
			    " Discarding position [$npos].");
		    $discarded_positions->{$npos} = 1;
		  }
	      }
	  }
      }

    verbose("Discarding high Z Score positions done.");
	...
	foreach my $nrec (grep {!exists($discarded_positions->{$_->[1]})}
			  @{$neighbors->{$mother_id}})
	  {
	    #error sum->{substitution type} += abundance of neighboring sequenc
	    $error_sum->{"$nrec->[2]>$nrec->[3]"} +=
	...
	verboseOverMe("Abund: [$total_abundance] Bases: [",
		      ...
		      "] ErrTypes: [",join(',',values(%$error_sum)),"]");

	#Foreach substitution type
	foreach my $subst_type (keys(%$error_sum))
	  {
	    my($orig_base);
	    if($subst_type =~ /^(.)/)
	      {$orig_base = uc($1);}
	...
	verbose("Estimated error rates for mother sequence [$mother_id]:\n\t",
		join("\n\t",
		     map {"$_:" .
			    (exists($estimated_error_rates->{DATA}
				    ->{$mother_id}->{$_}) ?
    ...
    verbose("\nAverage estimated error rates\n\t",
	    join("\n\t",
		 map {"$_:" .
			(exists($estimated_error_rates->{AVE}->{$_}) ?
			 sigdig($estimated_error_rates->{AVE}->{$_},$sigdig) :
			 '0')}
		 sort {$a cmp $b}
		 @subst_types));

    verbose("Error rate estimates of most abundant seqs done.");
    ...
    foreach my $mother_id (keys(%$seq_hash))
      {
	...
	foreach my $nrec (@{$neighbors->{$mother_id}})
	  {
	    #next if neighboring sequence is more abundant than mother sequence
	    next if(!$reverse_spillover && $abundance_hash->{$nrec->[0]} >
		    $abundance_hash->{$mother_id});

	    #n0hash->{neighboring sequence ID} += total abundance * estimated
	    #error rate->{substitution type}
	    $nzero_hash->{$nrec->[0]} += $abundance_hash->{$mother_id} *
	      $estimated_error_rates->{AVE}->{"$nrec->[2]>$nrec->[3]"};
	...
	print("$seq_hash->{$seq_id}->[0]$tmp_delim_with_val\n",
	      "$seq_hash->{$seq_id}->[1]\n");
      }
...
end_print
    ...
    if(!$advanced)
      {
	print << "end_print";
* WHAT IS THIS: Use this script when you have a metagenomic sample of ungapped
                aligned sequences, all of the same size, to help give you an
                idea which sequences are real and which are the result of PCR
                substitution errors.  This script takes a set of sequences and
                a list of their hamming distance 1 substitution neighbors (i.e.
                sequences that differ by a single nucleotide substitution)
                generated by hamming1neighbors.pl and computes N0 for each
                sequence (the expected abundance if the sequence was an error
                of another sequence).  The abundance value is appended to the
                defline of the output sequence file.  If a sequence is a "real"
                sequence and not an error derived from a real sequence, its
                abundance will be significantly higher than N0.
...
* INPUT FORMAT: Fasta or fastq format file containing a set of unique,
                ungapped, aligned, and same-sized sequences and with a unique
                identifier followed by an abundance value on the defline.
                Default defline format looks like this example:

                >lib_6;size=1002;

                Any defline format can be used as long as it is consistent,
                both pieces of information are present, and the identifier is
                the first unbroken string after the defline character.  The
                unique ID may contain the abundance value.  Use -p to extract
                the abundance values from deflines not in the above format.
...
sub usage
  {
    ...
    if($no_descriptions)
     ...
     -z|--z-threshold     REQUIRED This script uses a Z Score during the
                                   estimation of the error rate from the most
                                   abundant sequence's neighbors (See -e).  It
                                   ...
                                   neighbor, Amu is the average neighbor
                                   abundance, and Astddev is the standard
                                   deviation of neighbor abundance.  This
                                   option is the threshold to use to exclude
                                   "real" sequences from the estimation of the
                                   error rate.  Z scores above this threshold
                                   will be excluded from the error rate
                                   estimate.  A value of 0 means "exclude
                                   nothing".
     -t|--filetype        OPTIONAL [auto](fasta,fastq,auto) Input file (-i)
                                   type.  Using a value other than auto will
     ...
     -e|--num-estimate    OPTIONAL [10] A number of sequences in the input
                                   sequence file (-i) on which to base the
                                   calculation of the error rate estimates.
                                   The most abundant sequences will be used.
     -r|--use-reverse-    OPTIONAL By default, when calculating N0, neighbors
