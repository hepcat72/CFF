...
This version works best with Illumina data. The package will also work with 454 data, but improvements planned for the near future will significantly improve the support of this sequencing platform; contact us for details. We have not tested CFF on data from other platforms (IonTorrent, etc.); if you have such data and are interested in trying CFF, please contact us (see below).
...
The pipeline is an 8-step process. It can be run with a single command (run_CFF_on_FastA.tcsh or run_CFF_on_FastQ.tcsh), but each script component is also available.
...
The pipeline is designed to filter for errors that are the result of sequencing or PCR substitutions.  However, an additional post-filtering step has been provided to identify possible indels. This step is optional when applied to Illumina data, but required for 454 where indel errors are commonly generated during sequencing.
